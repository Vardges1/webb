...
<body>
    ...
    <div class="reg" style="margin-right:20%;margin-left:34%; margin-top: 0%; ">

  <div id="main">
   
<?php 
//Проверяем, если пользователь является админом, то ему будет показан блок в if-е
...
//Выбираем из БД все самолеты 
    ...
    $result_set57 = $mysqli->query("SELECT * FROM category");
...
?>
  ...
  <div class="news-card">
    ...
    <form method="post" action="?add">
        ...
        <p> Название категории <input name="sim" type="text">
...
<form method="post" action="?adds">
    <h2>Добавить симптом</h2>
     <p> Название симптома <input name="sim" type="text"></p>
  <p> Выберите, в какую категорию отнести симптом:<select size="3" name="cat">   <option disabled>Выберите категори</option><?php  while($row7 = mysqli_fetch_array($result_set57)){
      
?>
    <option value="<?php echo $row7['id_cat'];?>" selected><?php echo $row7['name'];?></option>
<?php } ?>  
   </select>
   ...
   <form method="post" action="?addsi">
    <h2>Добавить болезнь</h2>
     <p> Название болезни <input name="name" type="text"></p>
...
{ 
  ...
  if (mysqli_connect_errno()) {
    ...
    $result_set = $mysqli->query("INSERT INTO `category` VALUES (NULL,'$cat')");
    ?><script>alert("Категория добавлена!");</script>
...
$sim = $_POST['sim'];
  ...
  if (mysqli_connect_errno()) {
    ...
    $result_set = $mysqli->query("INSERT INTO `simptoms` VALUES (NULL,'$sim','$cat')");
    ?><script>alert("Симптом добавлен!");</script>
    <script>document.location.href="reges.php"
    </script><?php
    }
...
{ 
 ...
 $mysqli = @new mysqli('localhost', 'id15267461_root', '^EtgwnE9yjma%s}L', 'id15267461_base3');
  if (mysqli_connect_errno()) {
    ...
    $result_set = $mysqli->query("INSERT INTO `sick` VALUES (NULL,'$name',0)");
    $result_set78 = $mysqli->query('SELECT * FROM sick WHERE name="'.$name.'"');
    $row78 = mysqli_fetch_array($result_set78);
    $name1=$row78['id_sick'];
$aDoor=$_POST['f'];
...
for($i=0; $i < $N; $i++)
    {
        $result_set = $mysqli->query("INSERT INTO `sick_simp` VALUES ('$name1',$aDoor[$i])");
        
    }
   
    ?><script>alert("Болезнь добавлена!");</script>
    <script>document.location.href="reges.php"
    </script><?php
    }
?>
...
{ 
    $_SESSION['admin'] = false;
        $_SESSION['name'] = false;
    ?><script>document.location.href="reges1.php"</script><?php
  }
?>
